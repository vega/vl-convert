    ...
    R: Send + 'static,
{
    ...
    Python::attach(|py| py.detach(move || PYTHON_RUNTIME.block_on(make_future(converter))))
}
...
where
    Fut: Future<Output = PyResult<Py<PyAny>>> + Send + 'static,
{
    pyo3_async_runtimes::tokio::future_into_py::<_, Py<PyAny>>(py, fut)
}
...
where
    ...
    R: Send + 'static,
    C: FnOnce(Python<'_>, R) -> PyResult<Py<PyAny>> + Send + 'static,
{
...
pub fn parse_json_spec(vl_spec: Py<PyAny>) -> PyResult<serde_json::Value> {
    Python::attach(|py| -> PyResult<serde_json::Value> {
        ...
        } else if let Ok(vl_spec) = vl_spec.cast_bound::<PyDict>(py) {
...
pub fn parse_optional_config(config: Option<Py<PyAny>>) -> PyResult<Option<serde_json::Value>> {
...
pub fn parse_spec_to_value_or_string(vl_spec: Py<PyAny>) -> PyResult<ValueOrString> {
    Python::attach(|py| -> PyResult<ValueOrString> {
...
pub fn parse_format_locale(v: Py<PyAny>) -> PyResult<FormatLocale> {
    Python::attach(|py| -> PyResult<FormatLocale> {
        ...
        } else if let Ok(obj) = v.cast_bound::<PyDict>(py) {
...
pub fn parse_option_format_locale(v: Option<Py<PyAny>>) -> PyResult<Option<FormatLocale>> {
    match v {
        None => Ok(None),
...
pub fn parse_time_format_locale(v: Py<PyAny>) -> PyResult<TimeFormatLocale> {
    Python::attach(|py| -> PyResult<TimeFormatLocale> {
        if let Ok(name) = v.extract::<Cow<str>>(py) {
...
pub fn parse_option_time_format_locale(v: Option<Py<PyAny>>) -> PyResult<Option<TimeFormatLocale>> {
    match v {
        None => Ok(None),

...
| `/infoz` | Version metadata, supported Vega-Lite versions, and local timezone. |
...
- `GET` / `POST` / `PUT /admin/config/fonts/directories` for process font
  directory state.
- `GET` / `PUT /admin/config/fonts/cache` for the Google Fonts cache capacity and read-only directory.

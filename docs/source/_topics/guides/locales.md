...
A number locale follows the [d3-format locale definition](https://d3js.org/d3-format#locale_format). Save this example as `format-locale.json`:
...
::::{interface} cli
The option accepts a locale name, an inline JSON object, or a `.json` or `.jsonc` file:
...
::::{interface} server
The request field accepts the locale object directly. This complete body uses the same `chart.vl.json` input as the built-in locale example. Save it as `request.json`:

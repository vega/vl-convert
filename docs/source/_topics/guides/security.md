...
`--allowed-base-urls none` blocks every HTTP or HTTPS data and image URL and every filesystem path. Inline `data:` URLs remain allowed.
::::
...
Automatic Google Fonts makes network requests based on font names in a specification. Keep `auto_google_fonts` off unless you need it. If you enable it, set a variant threshold. See {doc}`fonts`.

Plugins run JavaScript and can import code from allowed domains. Prefer reviewed local startup plugins, and keep per-request plugins disabled for untrusted callers. See {doc}`plugins` and {doc}`../advanced/plugin-loading`.

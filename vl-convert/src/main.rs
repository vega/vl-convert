...
impl MissingFontsArg {
    fn to_missing_fonts_policy(self) -> MissingFontsPolicy {
        match self {
...
struct Cli {
    ...
    /// Automatically download missing fonts from the Fontsource catalog.
    #[arg(long, global = true)]
...
async fn main() -> Result<(), anyhow::Error> {
    ...
    let missing_fonts = missing_fonts_arg.to_missing_fonts_policy();
    use crate::Commands::*;
    match command {
        Vl2vg {
...
async fn install_fonts(fonts: &[String]) -> Result<(), anyhow::Error> {
    for family in fonts {
        install_font(family).await?;
    }
    Ok(())
}
